...
fun <T> Fragment.publishResults(key: String, result: T) {
    findNavController().previousBackStackEntry?.savedStateHandle?.set(key, result)   //     ->       publishResults(EXTRA_RANDOM_NUMBER, number)
}
...
fun <T> Fragment.listenResults(key: String, listener: ResultsListener<T>) {
    ...
    liveData?.observe(viewLifecycleOwner) { result ->
        if (result != null) {
            listener(result)
            liveData.value = null                   //   ->     listenResults<Int>(BoxFragment.EXTRA_RANDOM_NUMBER) {
        }
    }
}